...
# 5) Training (returns all three histories)
def train(model, optimizer, epochs=5000):
    ...
    return loss_history, l2_error_history, pde_residual_history
...
if __name__ == "__main__":
    ...
    if args.retrain:
        ...
        loss_history, l2_error_history, pde_residual_history = train(
        ...
        # load pretrained model + histories
    ...
    # 1) Solution
    axes[0,0].plot(x_test.numpy(), u_pred)
    axes[0,0].plot(x_test.numpy(), u_exact)
    axes[0,0].legend(["PINN", "Exact"])
    axes[0,0].set_title("Solution: PINN vs Exact")
    axes[0,0].set_xlabel("x")
    axes[0,0].set_ylabel("u(x)")
    axes[0,0].grid(True)

    # 2) Loss & PDE Residual & L2 Error
    epochs = list(range(1, len(loss_history) + 1))
    axes[0,1].plot(epochs, loss_history, label="Total Loss")
    axes[0,1].plot(epochs, pde_residual_history, label="PDE Residual (RMSE)")
    axes[0,1].plot(epochs, l2_error_history, label="L2 Rel Error")
    axes[0,1].set_yscale("log")
    axes[0,1].set_title("Training Metrics vs Epoch")
    axes[0,1].set_xlabel("Epoch")
    axes[0,1].set_ylabel("Value")
    axes[0,1].legend()
    axes[0,1].grid(True)

    # 3) PDE Residual across domain
    axes[1,0].plot(x_test.numpy(), residual_test)
    ...
    # 3) PDE Error across domain
    axes[1,1].plot(x_test.numpy(), u_pred - u_exact)
    axes[1,1].set_xlabel("x")
    axes[1,1].set_title("Solution Error")
