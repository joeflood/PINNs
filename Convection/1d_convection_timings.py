import torch
import torch.nn as nn              # ← separate import
import numpy as np
import matplotlib.pyplot as plt
torch.manual_seed(42)

# ----------------------------------------------------------------------
# PINN solver – returns total, PDE and BC loss histories
# ----------------------------------------------------------------------
...
# prepare x-grid ONCE, before the training loop
x_dense   = np.linspace(0, 1, 200)[:, None]
...
for eps in epsilons:
    model, _, Lpde, Lbc = pinn_collocation_solver(
        eps, layers, N_collocation=N_collocation, lr=lr, epochs=epochs)

    y_pred = model(x_dense_t).detach().cpu().numpy().ravel()
    y_exact = x_dense.ravel() + (np.exp(-(1 - x_dense.ravel()) / eps)
               - np.exp(-1 / eps)) / (np.exp(-1 / eps) - 1)

    sol_curves.append((eps, x_dense.ravel(), y_pred, y_exact))
...
for eps, Lpde, Lbc in loss_curves:
    ax_loss.plot(epoch_axis, Lpde, label=f'PDE ε={eps:g}')
    ax_loss.plot(epoch_axis, Lbc,  ls=':', label=f'BC ε={eps:g}')
